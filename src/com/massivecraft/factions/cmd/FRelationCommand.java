...
public abstract class FRelationCommand extends FCommand
{
	...
	private boolean breakAllies(Faction them) {
		...
		for (Faction otherFaction : Factions.i.get()) {
			if (otherFaction == myFaction || otherFaction == them) continue;
		...
		for (Faction otherFaction : allies) {
			if (otherFaction.getRelationTo(them).isAtLeast(Relation.ALLY)) {
				...
				otherFaction.msg("<i>Your faction is now &cenemy <i>to &c"+myFaction.getTag()+"<i> because they are at war with "+them.getTag(otherFaction));
