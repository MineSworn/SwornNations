...
public abstract class FRelationCommand extends FCommand
{
	...
	protected Map<Faction, Faction> nationWishes = new HashMap<Faction, Faction>();
	...
	public void perform()
	{
		...
		if (! them.isNormal())
		{
			msg("<b>You cant enemy %s.", them.describeTo(fme));
			return;
		}

		if (them == myFaction)
		{
			msg("<b>You cannot declare a relation to yourself.");
			return;
		}

		if (myFaction.getRelationWish(them) == targetRelation && targetRelation != Relation.NATION)
		{
			msg("<b>You already have that relation wish set with %s.", them.describeTo(fme));
			return;
		}
		...
		// Nations
		if (targetRelation == Relation.NATION)
		{
		...
		{
			if (! myFaction.playerHasPermission(fme, NPermission.NATION))
			{
				msg("<b>You don't have permission to break a Nation with %s.", them.describeTo(fme));
				return;
			}
		}
		...
		if (targetRelation.value == currentRelation.value)
		{
			...
			p.getServer().getPluginManager().callEvent(relationEvent);
		...
		if (targetRelation == Relation.ALLY && myFaction.playerHasPermission(fme, NPermission.NATION))
		{
			fme.msg("<i>Want a better relationship than ally? Use &b/f nation <i>to form an alliance only broken if both parties agree");
		}
	}
